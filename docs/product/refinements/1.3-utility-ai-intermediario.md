...
- [x] `ScheduleAction` usa `StartHour`/`EndHour` em vez de `Period`
- [x] Validação no load: schedule não contém Eating nem Sleeping
- [x] NPC tem `HomeLocation` e `EatingLocation` carregados do JSON
- [x] `ActiveScheduleAt(hour)` retorna ação correta ou false

**Needs:**

- [x] `Need` tem 4 campos (Hunger, Energy, Social, Security)
- [x] `world.Manager` tem `tickDuration` e expõe via `TickDuration()`
