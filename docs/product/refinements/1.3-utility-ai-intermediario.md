...
### 4.7 — Loneliness com lógica de companhia ✅ FEITO

- [x] Manager varre NPCs no mesmo `CurrentZone` antes de aplicar decay de Loneliness
- [x] Loneliness sobe quando NPC sozinho, decai quando acompanhado
- [x] Taxa de decay/recovery configurável

### 4.8 — Admin e debug ✅ FEITO

- [x] Comando admin `admin_get_npc_scores` retorna ranking de scores de um NPC
- [x] Comando admin `admin_get_npc_full` retorna estado completo do NPC
...
- [x] Loneliness decai quando sozinho, sobe com companhia
- [x] NPC com Loneliness alta procura outro NPC pra socializar

### 6.4 — Admin (próxima rodada)

- [x] `admin_get_npc_scores` retorna ranking + needs + schedule ativo
- [x] `admin_get_npc_full` retorna estado completo do NPC
